	...
	class Data_ListTest
	{
		public static void Show()
		{
			...
			{   //删除当前数据库中的所有Key  默认删除的是db0
                ...
                //client.PushItemToList(listid, JsonConvert.SerializeObject(liubei));
                ////只在内存中存储一秒
                //client.ExpireEntryAt(listid, DateTime.Now.AddSeconds(1));//2
                ...
                //3.从一个list的尾部移除一个数据，添加到另外一个list的头部，并返回移动的值
                //Console.WriteLine(client.PopAndPushItemBetweenLists(listid, "newlist"));//{"Id":3,"Name":"关羽","number":0}
                //Console.WriteLine("移动之后新队列的元素结果");
                //Console.WriteLine(client.GetItemFromList("newlist", 0)); //{"Id":3,"Name":"关羽","number":0}
